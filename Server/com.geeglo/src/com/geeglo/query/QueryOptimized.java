...
public class QueryOptimized {
	...
	public QueryOptimized(){
		try{
			dbManager1 = new DBManager("one_gram");
			dbManager2 = new DBManager("two_gram");
			dbManagerPageRank = new DBManager("page_rank");
	 ...
	 * 
	 */
	public ArrayList<String> parseQuery(String queryText){
	...
	private double computeSimilarity(Map<String, Double> query, Map<String, Double> document){
		...
		cosine = cosine/Math.sqrt(normalize_base_query * normalize_base_document);
		return cosine;
	}
	...
	private List<Document> sortByPageRankAndTfIdf(List<Document> resultDocList){
		Collections.sort(resultDocList, new Comparator<Document>(){
			...
			public int compare(Document  a, Document  b){
				double re = (b.getPagerank()*10 + normalizeTfIdf(b.getFirstTfIdf())) - (a.getPagerank()*10 + normalizeTfIdf(a.getFirstTfIdf()));
				if(re > 0){
					return 1;
				}else if(re == 0){
					return 0;
	...
	private List<Document> sortByPageRankAndCosine(List<Document> resultDocList){
		Collections.sort(resultDocList, new Comparator<Document>(){
			@Override
			public int compare(Document  a, Document  b){
				double re = (normalizePageRank(b.getPagerank())*100 + b.getCosine()) - (normalizePageRank(a.getPagerank())*100 + a.getCosine());	
				if(re > 0){
					return 1;
				}else if(re == 0){
					return 0;
	...
	/*
	 * sortByPageRank: sort the list of documents by page rank
	 * 
	 */
	private List<Document> sortByPageRank(List<Document> resultDocList){
		Collections.sort(resultDocList, new Comparator<Document>(){
			@Override
			public int compare(Document  a, Document  b){
				double re = b.getPagerank() - a.getPagerank();			
			if(re > 0){
					return 1;
				}else if(re == 0){
					return 0;
	...
	private double normalizePageRank(double page_rank_score){
		return (page_rank_score - 0.000007931610664546164)/0.106444550376911363836;
		
	}
	private double normalizeTfIdf(double tf_idf){
		// assuming the biggest tf = 10000;
		return tf_idf/18.33789029305773846281896290013;
	}
	/*
	...
	private List<Document> queryOneGram(List<String> query_words){
		...
		if(resultDocSet.size() != 0){		
			...
			//sortByTfIdf(resultDocSet, query_words.get(0));
			sortByPageRankAndTfIdf(resultDocSet);
			//sortByPageRank(resultDocSet);
		}
		return resultDocSet;
	}
	...
	public List<Document> queryTextOptimized(String queryText){
		...
		return document_final_list.size() < 10 ? document_final_list: document_final_list.subList(0, 10);
	}
}	
